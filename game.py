...
class Game:
    ...
    def update(self):
        ...
        if not self.demo_mode:
            if self.music_start_frame is True:
                pass
            elif self.music_start_frame is None:
                self.music_start_frame = pyxel.frame_count + _GRAPHICS_FPS * 11
            elif pyxel.frame_count > self.music_start_frame:
                self.music_start_frame = True
                loop = self.deadline == _ZEN_MODE
                sound_play(0, 'T115 @2 V50 O4 L1 R R D8 E8 D8 A8 D2  D8 E8 D8 A8 D#2  D8 E8 D8 A8 E2  R1  B-8 A8 G8 B-8 E2  A8 G8 F8 A8 D2  F8 E8 D8 F8 E8 D8 C#8 E8  <B-2 A2> D8 E8 D8 A8 D2  D8 E8 D8 A8 D#2  D8 E8 D8 A8 E2  R1  B-8 A8 G8 B-8 E2  A8 G8 F8 A8 D2  F4 G8 F8  E4 F8 E8  D4 E8 D8  A2 D8 E8 D8 A8 D2  D8 E8 D8 A8 D#2  D8 E8 D8 A8 E2  R1  B-8 A8 G8 B-8 E2  A8 G8 F8 A8 D2  F8 E8 D8 F8 E8 D8 C#8 E8  <B-2 A2> D8 E8 D8 A8 D2  D8 E8 D8 A8 D#2  D8 E8 D8 A8 E2  R1  B-8 A8 G8 B-8 E2  A8 G8 F8 A8 D2  F8 E8 D8 F8 G8 F8 E8 G8  A8 G8 F8 A8 G4 C4 A1  &A4 G4 F4 E4  D1  C2. C4 A1  &A4 G4 F4 E4  F1  G1 G1 F1 E1 D1 C1 A1 B-1 B-1 A1&A1 R1 R1 R1 R1 R1 R1 R1 R1', loop=loop)
                sound_play(1, 'T115 @2 V40 Q20 O6 L12 [ FD<A> D<AF AFD FD<A>> ] 2 O6 [ FD<A> D<AF AFD FD<A>> ] 2  FD<A> D<AF AEC# EC#<A>>  C#<AE AEC# EC#<A> C#<AE>   B-GD GD<B-> GEC EC<G>  >C<AF AFC B-GDGD<B->  >D<AF AFD >C<GE GEC  >C<FA> >C<FA> C#<AE C#<AE O6 [ FD<A> D<AF AFD FD<A>> ] 2  FD<A> D<AF AEC# EC#<A>>  C#<AE AEC# EC#<A> C#<AE>   B-GD GD<B-> GEC EC<G>  >C<AF AFC B-GDGD<B->  >D<AF AFD >C<GE GEC  B-FA B-FA> C#<AE C#<AE O6 [ FD<A> D<AF AFD FD<A>> ] 2  FD<A> D<AF AEC# EC#<A>>  C#<AE AEC# EC#<A> C#<AE>   B-GD GD<B-> GEC EC<G>  >C<AF AFC B-GDGD<B->  >D<AF AFD >C<GE GEC  >C<FA> >C<FA> C#<AE C#<AE O6 [ FD<A> D<AF AFD FD<A>> ] 2  FD<A> D<AF AEC# EC#<A>>  C#<AE AEC# EC#<A> C#<AE>   B-GD GD<B-> GEC EC<G>  >C<AF AFC B-GDGD<B->  >D<B-F B-FD >C<GE GEC  >C<FA C<FA> >C<GE CEG O6 Q60 >C<AF C<AF Q80 C2  R4 <B-4 A4 G4  A1  G1 O6 Q60 >C<AF C<AF Q80 C2  R4 <B-4 A4 C4  >D1  E1 E-1 D1 C1  <B-4 A4 G4 F4 G1 C1 D1 C1 C1&C1 R1 R1 R1 R1 R1 R1 R1 R1', loop=loop)
                sound_play(2, 'T115 @1 V50 Q80 O2 L4 V5D V10D V15D V20D  V25D V30D V35D V40D DDDD DDDD DDAA AAAA GGCC FFB-B- DDEE FFEE DDDD DDDD DDAA AAAA GGCC FFB-B- DDCC B-B-AA DDDD DDDD DDAA AAAA GGCC FFB-B- DDEE FFEE DDDD DDDD DDAA AAAA GGCC FFB-B- GGAA B-B-CC FFFF FFFF B-AGF EDC<G F>FFF FFFF B-AGB- >C<B-A>C C<B-A G  A G F A  G F E G  F E D C <B- A G >C  <F >F F F  F F F F  F F F F F F F F F1 R1 R1 R1 R1 R1 R1 R1 R1', loop=loop)
